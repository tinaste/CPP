...
int CMFCApplication5Dlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	AfxMessageBox(L"위도우 생성", MB_ABORTRETRYIGNORE | MB_ICONHAND);
...
void CMFCApplication5Dlg::OnDestroy()
{
	...
	AfxMessageBox(L"위도우 삭제", MB_YESNO | MB_ICONHAND);
}

CString str;
void CMFCApplication5Dlg::OnMButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.

	if (AfxMessageBox(_T("더블클릭"), MB_YESNO) == IDYES)
	{
		str = "더블클릭";
...
void CMFCApplication5Dlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	CString str;
	CTime time; //time cttime 클래스 객체 생성

	time = CTime::GetCurrentTime();
	str.Format(L"%d 시 %d 분 %d 초", time.GetHour(), time.GetMinute(), time.GetSecond());
...
void CMFCApplication5Dlg::OnRButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	SetTimer(1, 1000, NULL); //NULL이므로 ONTIMER가 호출됨.
	AfxMessageBox(L"타이머가 생성", MB_YESNO | MB_ICONHAND);
